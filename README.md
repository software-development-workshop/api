...
The API docs are then at <http://localhost:8000/docs>. Verification emails go out through
Resend and arrive in a real inbox, so `RESEND_API_KEY` has to be exported the same way —
ask the team for one, they are handed out per person through the password manager. Send to
`delivered@resend.dev` to exercise the flow without filling your own inbox.
