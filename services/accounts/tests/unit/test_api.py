...
def test_never_echoes_the_password_or_its_hash(client: TestClient) -> None:
    body = client.post("/registrations", json=VALID).json()
...
def test_rejects_a_missing_required_field(client: TestClient, missing: str) -> None:
    response = client.post("/registrations", json={k: v for k, v in VALID.items() if k != missing})
...
def test_rejects_a_handle_already_taken(
    ...
    assert client.post("/registrations", json=VALID).json()["type"].endswith("/handle-taken")


def test_verification_link_activates_the_account(client: TestClient, mailer: FakeMailer) -> None:
    client.post("/registrations", json=VALID)

    response = client.get(f"/verifications/{mailer.last_token}")
...
def test_verification_link_stops_working_after_it_is_used(
    ...
    client.post("/registrations", json=VALID)
    client.get(f"/verifications/{mailer.last_token}")
...
def test_rejects_a_token_nobody_issued(client: TestClient) -> None:
    assert client.get(f"/verifications/{tokens.generate()}").status_code == 400
...
def test_resend_sends_a_fresh_link(client: TestClient, mailer: FakeMailer) -> None:
    ...
    response = client.post("/verifications/resend", json={"email": VALID["email"]})

    assert response.status_code == 202
    assert mailer.last_token != first
    assert client.get(f"/verifications/{mailer.last_token}").status_code == 200
...
def test_resend_answers_the_same_for_an_unknown_address(
    ...
    # an account.
    known = client.post("/verifications/resend", json={"email": VALID["email"]})
    unknown = client.post("/verifications/resend", json={"email": "nadie@udesa.edu.ar"})
